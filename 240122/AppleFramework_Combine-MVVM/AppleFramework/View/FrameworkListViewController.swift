...
//    let items = CurrentValueSubject<[AppleFramework], Never>(AppleFramework.list)
    ...
    // ViewModel
    var frameworkListViewModel : FrameworkListViewModel!

    override func viewDidLoad() {
        super.viewDidLoad()
        frameworkListViewModel = FrameworkListViewModel(items: AppleFramework.list)
...
//        }.store(in: &subscriptions)
        ...
        frameworkListViewModel.items
            .receive(on: RunLoop.main)
            .sink { [unowned self] list in
                self.applySectionItems(list)
            }.store(in: &subscriptions)
        
        frameworkListViewModel.selectedItem
            .compactMap {$0}
            .receive(on: RunLoop.main)
            .sink { [unowned self] framework in
                let sb = UIStoryboard(name: "Detail", bundle: nil)
...
//        didSelect.send(framework)
        frameworkListViewModel.didSelect(at: indexPath)
    }
}
